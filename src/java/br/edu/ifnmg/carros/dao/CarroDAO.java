...
public class CarroDAO implements CrudDAO<Carro>{
    ...
    public void salvar(Carro carro) throws ErroSistema{
        try {
            ...
            if(carro.getId() == null){
                ps = conexao.prepareStatement("INSERT INTO `carro-table` (`modelo`,`fabricante`,`cor`,`ano`) VALUES (?,?,?,?)");
            } else {
                ps = conexao.prepareStatement("update carro-table set modelo=?, fabricante=?, cor=?, ano=? where id=?");
    ...
    public void deletar(Carro carro) throws ErroSistema{
        try {
            Connection conexao = FabricaConexao.getConexao();
            PreparedStatement ps  = conexao.prepareStatement("delete from carro-table where id = ?");
    ...
    public List<Carro> buscar() throws ErroSistema{
        try {
            Connection conexao = FabricaConexao.getConexao();
            PreparedStatement ps = conexao.prepareStatement("select * from carro-table");
